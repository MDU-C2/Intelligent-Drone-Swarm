...
\begin{itemize}
    ...
    \item Assigning initial sectors at initial mission set up.
    \item Setting up initial neighbourhoods.
...
\begin{itemize}
    ...
    \item One AgentID is reserved for the SwarmInterface and by extension the \gls{searchmana}.
...
\end{itemize}
A \textbf{returning} agent A must perform a registration procedure (see \cref{fig:ad-agent-rejoin}):
\begin{enumerate}
...
\end{enumerate}
A \textbf{new} agent B must perform a registration procedure (se \cref{fig:ad-new-agent}):
\begin{enumerate}
...
\Cref{fig:bdd-context} depicts that:
\begin{itemize}
    \item One swarm contains four to 255 agents.
    \item One \gls{agent} contains one \acrshort{fmu}. 
    \item \acrshortpl{uav} inherit from agent, meaning that \acrshortpl{uav} must contain \acrshortpl{fmu}. 
    \item Each agent acts according to a state machine. 
    \item Each agent executes the protocol module. 
    \item The protocol module carries out a set of behaviours. 
    \item \glspl{searchmana} use a Swarm Interface to interact with a swarm.
\end{itemize}
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{SD-01_BDD - Protocol Context 2.0.png}
    \caption{\textit{(SD-01)} \acrfull{bdd} of which systems and diagrams the protocol module is related to.}
