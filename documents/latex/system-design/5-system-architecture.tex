...
\end{itemize}
Each fully operational agent maintains a neighbourhood of four \textbf{fully operational} agents in total consisting of itself and its three geographically closest agents (where ''closest'' is defined using the centres of each agent’s initial \glspl{sector}), and any partially operational agents.
...
\begin{itemize}
    \item Initialisation data (e.g. initial sector map, intended swarm size, initial neighbourhoods, initial sector allocation).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        %\item[$\circ$] List of Sectors (i.e., search area divided into sectors, and sectors valued using hot regions).
