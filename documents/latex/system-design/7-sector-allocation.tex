...
\begin{itemize}
    \item Sectors have values.
    %\item Sectors have discounts.
    \item Agents have budgets.
    \item Agents ''purchase'' sectors they can search efficiently.
...
\begin{itemize}
    ...
    \item Each agent evaluates the discount of a sector using a \textbf{personal discount function}.
    \item Each agent evaluates the desirability of a sector using a \textbf{personal desirability function}.
    ...
    \item Each agent bid on sectors in order of greatest desirability, i.e., they bid for sectors they can search efficiently.
...
\subsection{Sector Discount, Desirability \& Cost}
...
\end{enumerate}
From the probability, each sector is then given a value where high-probability sectors receive a higher value, and low-probability sectors receive a lower value. 
%Thereafter, for each agent A and sector S, an an additional cost component is computed based on the distance between A's position at the time of entering Bidding state (\cref{fig:fully-op}) and the centre of S. Agents use this cost when deciding which sectors to bid for.

Each agent, when bidding for sectors, calculate the distances from their position when entering Bidding state (\cref{fig:fully-op}) to the middle of each available sector, and then assign the following values to all available sectors: Discount, desirability, and cost.

\textbf{Distance} between an agent and a sector is calculated by subtracting the middle position of a sector from the Agent's position when entering Bidding state (\cref{fig:fully-op}):
\begin{equation}
    distance(S_i) = | agentPos(x, y) - sectorMid(x_i, y_i) | = \sqrt{(x - x_i)^2 + (y - y_i)^2}
\end{equation}

\textbf{Discount} is calculated by dividing distance with the diagonal length of the Search Area:
\begin{equation}
    discount(S_i) = 1 - \frac{distance(S_i)}{diagonal(SearchArea)}
\end{equation}

where the diagonal of the search area can be calculated by using its northeast (ne) and southwest (sw) corners:
\begin{equation}
    diagonal(SearchArea) = \sqrt{(x_{ne} - x_{sw})^2 + (y_{ne} - y_{sw})^2}
\end{equation}

\textbf{Desirability} is calculated by multiplying a sector's value by its discount:
\begin{equation}
    desirability(S_i) = value(S_i) \times discount(S_i)
\end{equation}

\textbf{Cost} of a sector is calculated by using the sector's value, discount, and how much energy is spent ($k$) flying the distance to the sector:
\begin{equation}
    cost(S_i) = value(S_i) \times (1-discount(S_i)) + k \times distance(S_i)
\end{equation}

Each agent bids for sectors in order of highest desirability, which means that desirability encourages the Sector Allocation Market to favour assigning high-probability sectors before low-probability sectors while simultaneously encouraging agents closer to the sectors to attempt purchasing them before other agents.

%sector S is assigned a cost that reflects both how important it is and how hard it is for a particular agent to search it.

Future work must decide the algorithm for value $k$, and then verify if equations 1 to 5 work as intended.
...
After the first sector, agents may attempt to purchase additional sectors using the Sector Allocation Market (\cref{fig:ad-initial-market}).
Each available sector's desirability and cost are calculated locally by each agent that wants to bid for sectors, and the agents then evaluate which sectors are attractive based 
on desirability, cost, and remaining budget. Then, for each candidate sector S, an agent creates an Origin rumour expressing its intention to buy S (using an 
...
threshold $\alpha$ is met, and then determine if their purchase was successful. Winning agents broadcast decision-Origins to announce their purchases (e.g. ''I own sector S'').
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{SD-19_AD - Initial Sector Allocation Market 1.0.png}
...
All changes in \gls{sector} ownership (initial purchases, reassignments, and releases) are confirmed through the Rumour Mill to avoid conflicting views. A typical sequence for a successful purchase of sector S by agent A is (\cref{fig:ad-market}):
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item If the bid is a success, A broadcasts a new Origin rumour announcing their purchase, where success → ownerID(S) = A and A's budget is reduced, and failure → no change to ownerID(S) and A's budget stays the same.
    \end{itemize}
\end{enumerate}
...
When an agent becomes degraded (e.g. bad LiDAR):
\begin{enumerate}
    ...
    \item Any agent (including neighbours) that receives three Sources updates A's sectors to unassigned, and creates an Opinion rumour (e.g. ''A should become SecondaryRelay'').
    \item The agent collects Opinion rumours; Once Opinion rumours from$\geq \alpha$ fraction of agents have arrived, the agent internally decides to switch to SecondaryRelay.
...
Broadcast communication is lossy; messages may be dropped or delayed. As a result, a bidding agent might not receive enough Source or Opinion rumours within a reasonable time, or some agents may temporarily disagree about whether a bid is still active. To handle this, simple recovery strategies can be used:
\begin{itemize}
    \item Timeout and retry - if the bidder does not receive enough Source or Opinion rumours within a timeout, it may rebroadcast the bid Origin rumour.
...
\subsection{Bidding Process After Initial Sector Allocation}
% Fix title and where to place later
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{SD-28_AD - Sector Allocation Market 1.0.png}
