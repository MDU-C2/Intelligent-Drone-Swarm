...
\end{itemize}
Each agent maintains a neighbourhood of exactly four agents in total consisting of itself and its three geographically closest agents (where ''closest'' is defined using the centres of each agent’s initial \glspl{sector}).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/System-Design/SD-24_AD - Initial Neighbourhoods 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\linewidth]{figures/System-Design/SD-25_AD - Recalculate Neighbourhoods 1.0.png}
...
Each agent is a \acrshort{uav} equipped with standard flight systems (example found in \cref{fig:bdd-uav}) and a protocol module for swarm-level coordination. A simplified \acrfull{ibd} (\cref{fig:IBD-agent}) models the internal structure relevant for the protocol module.

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-10_IBD - Agent 1.0.png}
    \caption{\textit{(SD-10)} \acrshort{ibd} of \glspl{agent}' internal structure. Even though flowports are named P2P\_Rx / P2P\_Tx in the diagram, they conceptually represent broadcast send/receive on a shared channel. Flow specifications for the flowports can be seen in \cref{fig:flowspec}.}
    \label{fig:IBD-agent}
\end{figure}
...
A simplified architecture of the protocol is defined by \cref{fig:bdd-protocol}. 

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-13_BDD - Protocol 1.0.png}
    \caption{\textit{(SD-13)} \acrshort{bdd} of which main parts the protocol module contains. DynamicInfo holds lists containing information about \glspl{agent} in the swarm (where the first index is always the \gls{agent} itself) and about \glspl{sector}. StaticInfo holds enumerations of message types, health statuses, and tasks.}
    \label{fig:bdd-protocol}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-16_IBD - Protocol 1.0.png}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[$\circ$] Tracking Opinion rumours and detect when the chosen consensus threshold $\alpha$ (e.g. 70\% in examples) is reached for a given Origin.
    ...
    \begin{itemize}
        \item[$\circ$] Implementing the agent’s state machine (start, task, search, found, confirmed, bad\_sensor, secondary\_task, return, failed).
        \item[$\circ$] Mapping health status, sector ownership, and commands into roles (search, relay, return, etc.).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/System-Design/SD-15_IBD - Protocol BlackBox 1.0.png}
    \caption{\textit{(SD-15)} Blackbox view of the protocol. Flowports named ui\_Rx and ui\_Tx are to receive broadcasts from and broadcast to SwarmInterface. Flowports named P2P\_Rx and P2P\_Tx are to receive broadcasts from and broadcast to other \glspl{agent}. The flowport named faultManUnit is for receiving messages from the \acrshort{fmu}. Flow specifications for the flowports can be seen in \cref{fig:flowspec}.}
    \label{fig:ibd-protocolBB}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-09_FlowSpecifications - RxTx 1.0.png}
    \caption{\textit{(SD-09)} Flow specification for flowports. ROr means Rumour Origin, RS means Rumour Source, and ROp means Rumour Opinion.}
    \label{fig:flowspec}
\end{figure}
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[$\circ$] Join/rejoin announcements for an Originator.
        \item[$\circ$] Subject found.
    \end{itemize}
    \item Opinion rumours, such as:
    \begin{itemize}
        \item[$\circ$] Agent X won bid for sector Y.
...
\begin{itemize}
    \item Initialisation data (e.g. initial sector map, intended swarm size).
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item[$\circ$] neighbourhood memberships,
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/System-Design/SD-14_IBD - StaticInfo 1.0.png}
...
\begin{itemize}
    ...
    \end{itemize}\color{red}
    \item SectorCost entries (per sector):
    \begin{itemize}
        \item[$\circ$] sectorID: Unique ID of sector S.
        \item[$\circ$] value: Value based on the probability that the Subject will be found in sector X. Probability is based on Hot Regions.
        \item[$\circ$] initSector: Latitude and longitude of the first initial sector of the owner.
        \item[$\circ$] cost: Cost (f(distance, value)) of sector X based on value and initSector. Calculated by TaskAllocation.
    \end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/System-Design/SD-05_DataTypes 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-06_RumourOrigins 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-07_RumourSources 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-08_RumourOpinions 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/System-Design/SD-11_BDD - SwarmInterface 1.0.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.6\linewidth]{figures/System-Design/SD-12_IBD - SwarmInterface BlackBox 1.0.png}
...
From the protocol module perspective, the SwarmInterface provides an input channel for high-level commands and mission configuration (initial \gls{sector} map, Hot Regions), and an output channel for aggregated swarm status and event reports.
...
The \gls{secmarket} and Task \& Role logic run on top of this communication fabric to allocate work and determine agent behaviours.



\begin{figure}[H]
    \centering
    \includegraphics[width=\linewidth]{figures/System-Design/SD-21_IBD - Communication Context 1.0.png}
    \caption{\textit{(SD-21)} \acrshort{ibd} over communication context. Even though flowports are named P2P\_Rx / P2P\_Tx in the diagram, they conceptually represent broadcast send/receive on a shared channel. Flow specifications for the flowports can be seen in \cref{fig:flowspec}.}
