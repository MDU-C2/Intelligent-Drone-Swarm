...
Thereafter, for each agent A and sector S, an an additional cost component is computed based on the distance between the centre of A’s initial sector and the centre of S. Agents use this cost when deciding which sectors to bid for.

Future work must decide the exact algorithm for this.

\subsection{Agent Budgets \& Budget Dynamics}
\label{subsec:agent-budget}
Each agent A is assigned a budget B$_A$, which is primarily based on its battery level. The budget represents how many and which sectors the agent can afford to purchase. At mission start (or when joining), an agent’s initial budget is set based on its available battery meaning that higher battery has a larger budget, and lower battery has a smaller budget.

Budget dynamics follow these rules:
\begin{itemize}
    \item When agent A successfully purchases a \gls{sector} S through the \gls{secmarket}, B$_A$ decreases by the cost of S.
    \item When agent A finishes searching sector S and confirms completion via the \gls{rumourmill}, B$_A$ increases by the cost of S (but does not go over what is allowed by the battery level).
    \item Agent A may only bid on sectors if its remaining budget is sufficient to cover the sector’s cost.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figures/MDU_logotyp.png}
    \caption{\textit{(SD-19)} \gls{actdia} showing the process of bidding for sectors after initial sectors have been assigned.}
...
\begin{itemize}
    \item Uses probability information (favour high-probability regions).
    %\item takes into account agent starting positions where available,
    \item Ensures that every agent starts with something to do.
\end{itemize}
...
\subsubsection{Initial Auction Loop}
After the first sector, agents may attempt to purchase additional sectors using the Sector Allocation Market:
\begin{enumerate}
    \item Each agent evaluates which sectors are attractive based on its cost function and remaining budget.
    \item For each candidate sector S, the agent creates an Origin rumour expressing an intention to buy S (using an appropriate msgID).
    \item Neighbours interpret this bid and broadcast Source rumours evaluating:
    \begin{itemize}
        \item Whether S is currently unassigned or conflicts with another owner,
        \item whether the bidding agent’s budget appears sufficient,
        \item any other validity conditions.
    \end{itemize}
    \item Once any agent sees three Source rumours for this Origin rumour, it creates an Opinion rumour summarising the interpretation (e.g. “bid valid” or “bid invalid”).
    \item The bidding agent collects Opinions until the consensus threshold $\alpha$ is met, then decides whether the bid succeeded.
    \item The bidding agent broadcasts a decision-Origin announcing the outcome (e.g. “I own sector S” or “bid rejected”).
\end{enumerate}
...
\subsection{Activity Diagram SD-29}
% Fix title and where to place later
\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{figures/MDU_logotyp.png}
    \caption{\textit{(SD-29)} \gls{actdia} of the SectorAllocationMarket after the initial bidding process is complete.}
    \label{fig:ad-market}
\end{figure}