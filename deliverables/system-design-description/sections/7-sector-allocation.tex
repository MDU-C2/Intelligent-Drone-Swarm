...
\begin{itemize}
    ...
    \item Each agent evaluates the distance to a sector using a \textbf{personal discount function}.
...
Each agent, when bidding for sectors, calculate their distance to each sector, and then assign the following values to all available sectors: Discount, desirability, and cost.

Distance between an agent and a sector is calculated by subtracting the middle position of a sector from the Agent's position when entering Bidding state (\cref{fig:fully-op}):
\begin{equation}
    distance(S_i) = agentPos(x, y) - sectorMid(x_i, y_i)
\end{equation}

Discount is calculated by dividing distance with the diagonal length of the Search Area:
\begin{equation}
    discount(S_i) = 1 - \frac{|distance(S_i)|}{diagonal(SearchArea)}
\end{equation}

Desirability is calculated by multiplying a sector's value by its discount:
\begin{equation}
    desirability(S_i) = value(S_i) \times discount(S_i)
\end{equation}

Cost of a sector is calculated by using the sector's value, discount, and how much energy is spent per flown unit distance:
\begin{equation}
    cost(S_i) = value(S_i) \times (1-discount) + k \times distance
\end{equation}
