...
\end{enumerate}
The protocol is designed to tolerate approximately one Byzantine fault per neighbourhood and multiple crash or omission faults swarm-wide.
...
\begin{itemize}
    ...
    \item Once the Rumour Mill confirms that A is missing or failed:
    \begin{itemize}
        \item[$\circ$] A is set as no longer a member in AgentInfo,
        \item[$\circ$] neighbourhoods are recalculated to exclude A and retain 1 + 3 formation of fully operational agents,
        \item[$\circ$] sectors previously owned by A are considered released and re-enter the \gls{secmarket} (\Cref{subsubsec:sector-release-from-failed}).
    \end{itemize}
\end{itemize}
...
\begin{itemize}
    \item Health degradation $\rightarrow$ Secondary roles:
    \begin{itemize}
        \item[$\circ$] bad sensor $\rightarrow$ transition from PrimarySearch in FullyOperational to appropriate task in PartiallyOperational, prevents faulty sensors from corrupting search results.
        \item[$\circ$] Release of \glspl{sector} via the Sector Allocation Market.
        \item[$\circ$] Stop performing primary search tasks.
    \end{itemize}
    \item Critical failure $\rightarrow$ transition to Failed:
    \begin{itemize}
        \item[$\circ$] Severe \acrshort{fmu} errors or complete communication loss $\rightarrow$ Failed, and possibly Returning or \\EmergencyLanding.
        \item[$\circ$] Removal of the agent from neighbourhoods and SectorInfo after Rumour Mill confirmation.
    \end{itemize}
    \item Low battery $\rightarrow$ PartiallyOperational/Returning:
    \begin{itemize}
        \item[$\circ$] Low-battery event $\rightarrow$ sector release, then PartiallyOperational and Returning.
        \item[$\circ$] Prevents agents from purchasing sectors and dying silently in the field with sectors assigned.
    \end{itemize}
    \item Subject confirmation $\rightarrow$ Returning
    \begin{itemize}
        \item Subject confirmed found $\rightarrow$ all agents transition toward Returning.
    \end{itemize}
\end{itemize}
...
Each fully operational agent forms a neighbourhood consisting of itself and its three geographically closest (based on initial sectors) agents that are also fully operational. Thus, each neighbourhood contains four fully operational agents in total (1 + 3), along with any partially operational agents. 
Four fully operational agents is required for subject confirmation to work as intended, and each agent must be seen as a neighbour by at least three other agents for missed pulses to be registered properly using the Rumour Mill.

Neighbourhoods overlap, meaning that a given agent is the centre of its own neighbourhood, and is one of at least three neighbours in three or more other agents' neighbourhoods.
...
\begin{itemize}
    ...
    \item Any agent that receives three Source rumours for this Origin rumour can interpret the event through voting, and possibly broadcast an Opinion rumour.
\end{itemize}
Within the originator's neighbourhood, there are at least three neighbour Source rumours (from its neighbours). To interpret an event and possibly broadcast an Opinion rumour, an agent needs three Source rumours from three distinct source agents, and those Source rumours may come from neighbours and/or other agents in the swarm - they are not required to come from the three distinct source agents, they might as well come from one agent in broadcasting range.

\vspace{0.2cm}

\subsubsection{Tolerance of One Byzantine Neighbour}
Consider a single neighbourhood of 4 fully operational agents (A = originator, plus neighbours B, C, D), and assume at most one of these neighbours is Byzantine:
\begin{itemize}
...
\end{itemize}
Since Opinions are derived from multiple independent Source rumours, a single Byzantine neighbour cannot, by itself, force a consistently incorrect 
Opinion if the Opinion logic respects the majority of honest Source rumours. Therefore with neighbourhood size 4 (1 + 3), the design can tolerate one 
Byzantine agent per neighbourhood without corrupting the consensus process, assuming that Opinion formation logic is dominated by honest Source rumours.
...
The exact value of $\alpha$ is left for future work.
...
\begin{itemize}
    ...
    \begin{itemize}
        \item[$\circ$] The state machine defines allowed transitions (e.g. from PrimarySearch to Returning, not both at once).
...
\begin{itemize}
    ...
    \begin{itemize}
        \item[$\circ$] Faulty behaviour from one agent is limited by neighbourhood cross-checking and the need for multiple Source rumours and Opinion rumours.
        \item[$\circ$] Degraded agents are moved into PartiallyOperational or Failed.
    \end{itemize}
    ...
    \begin{itemize}
        \item[$\circ$] Because logic is local and rumour-based, temporary disconnects do not permanently break the system; Agents can rejoin and resynchronise through proper registration procedure.
    \end{itemize}
\end{itemize}
