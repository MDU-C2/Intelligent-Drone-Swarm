...
\begin{itemize}
    ...
    \begin{itemize}
        \item[$\circ$] requires three independent Source rumours before an agent A interprets the event from agent Y through internal voting, and
        \item[$\circ$] after event interpretation, agent A possibly broadcasts an Opinion, where
        \item[$\circ$] agent Y (the originator) receives Opinions up to a certain consensus threshold $\alpha$ before making a final decision.
    \end{itemize}
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/System-Design/SD-03_AD - Swarm Registration 2.0.png}
    \caption{\textit{(SD-03)} \gls{actdia} of swarm registration at initial start-up. This activity diagram does not cover agents joining the swarm after mission start.}
    \label{fig:Q5RMtrace}
\end{figure}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/System-Design/SD-04_AD - Search Area Division 1.0.png}
...
\color{red}Activity diagram of the process of initial sectors being assigned. Includes Search Manager marking Hot Regions, SwarmInterface dividng the map into sectors, Sectors are assigned probability values, SectorInfo is transmitted to agents.\color{black}

\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/System-Design/SD-20_AD - Initial Sectors 1.0.png}
...
\color{red}Activity diagram showing the process of agents joining after mission start\color{black}

\subsubsection{Agent Rejoin}
%Agent re-joining after temporary loss
\color{red}Activity diagram showing the process of agents re-joining after mission start\color{black}
